...
class SSD(object):
    _defaults = {
        "model_path"        : 'model\Optimal-MRI-selection.h5',
        "classes_path"      : 'model\medical_classes.txt',
    ...
    def detect_image(self, image):
        ...
        #   将预测结果进行解码
        #-----------------------------------------------------------#
        results = self.bbox_util.detection_out(preds, confidence_threshold=self.confidence, keep_top_k=3)
        ...
        #-----------------------------------------------------------#
            ...
            for i, c in enumerate(top_label_indices):
                ...
                results_info = np.concatenate([np.expand_dims(int(c)-1,-1), np.expand_dims(score,-1), np.expand_dims(top,-1), np.expand_dims(left,-1), np.expand_dims(bottom,-1), np.expand_dims(right,-1)], axis=-1)
